...
exports.type_details = function (req, res, next) {
    Instrument.find({ type: req.params.id }).exec(function (err, result) {
        if (err) {
            next(err);
        }
        res.render('type_details', {
            title: 'Similar Types',
            list: result,
            url: req.params.id,
        });
...
exports.delete_type_post = function (req, res) {
    Instrument.find({ type: req.params.id }).exec(function (err, results) {
        if (results.length > 0) {
            res.render('type_delete', {
                title: 'Delete Type',
                error: 'You must delete instruments tied to this type!',
                results,
            });
        } else {
